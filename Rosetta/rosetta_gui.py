...
class Init_Window():
    ...
    def init(self):
        ...
        #创建日志文本框
        self.log_Text = Text(self.window, width=100, height=10)
        self.log_Text.place(x=10,y=50)
        ...
        #生成密钥按钮
        self.create_key_button = Button(self.window, text="生成密钥", width=10,command=self.create_keys_click)  # 调用内部方法  加()为直接调用
        self.create_key_button.place(x=190,y=200)

        #导入私钥按钮
        self.import_key_button = Button(self.window, text="导入私钥", width=10)  # 调用内部方法  加()为直接调用
        self.import_key_button.place(x=280,y=200)
